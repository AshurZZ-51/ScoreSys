...
import { buildMaterialUpsert, getMaterialItemsForRound, getMaterialStatus, isMaterialStatus } from '@/lib/projectPoolWorkflow';
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  if (!isProjectPoolV2Enabled()) return NextResponse.json({ error: '项目池功能尚未启用' }, { status: 404 });
  try {
    const { item_key, status, note = '', round_no: requestedRoundNo = 1 } = await request.json();
    const roundNo = Number(requestedRoundNo) === 2 ? 2 : 1;
    ...
    const materialDefinition = getMaterialItemsForRound(roundNo).find((item) => item.item_key === item_key);
    ...
    const { error } = await supabaseAdmin.from('project_materials').upsert(
      buildMaterialUpsert(params.id, item_key, materialDefinition.required, status, note, session.code, now, roundNo),
      { onConflict: 'project_id,round_no,item_key' }
    );
