...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await supabaseAdmin.from('project_status_history').insert({ project_id: params.id, event_type: 'material_checked', from_status: current.status, to_status: current.status, operator_code, note: `${item_key}: ${status}${note ? `；${note}` : ''}` });
