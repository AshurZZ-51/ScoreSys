...
import { getMaterialItemsForRound, isMaterialStatus } from '@/lib/projectPoolWorkflow';
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await upsertProjectMaterial(id, item_key, materialDefinition.required, status, String(note || ''), session.code, now, roundNo);
