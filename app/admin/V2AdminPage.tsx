...
export default function V2AdminPage() {
  ...
  const schedule = async (projectIds: string[], meetingId: string) => {
    ...
    const rounds = selected.map((project) => project.status.includes('r2') ? 2 : 1).filter((round, index, all) => all.indexOf(round) === index);
    if (rounds.length !== 1) { setNotice('请按评审轮次分别安排项目。'); return; }
    const response = await fetch('/api/meeting-assignments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ meeting_id: meetingId, pool_project_ids: projectIds, round_no: rounds[0], operator_code: adminCode() }) });
  ...
  const refreshProject = async (updated?: AnyRecord) => {
    if (updated?.id) setProjects((current) => current.map((project) => project.id === updated.id ? { ...project, ...updated } : project));
    await load();
    if (updated?.id && !updated.archived_at) await openProject(updated);
  };
  ...
  return <main style={styles.page}>
    ...
    {tab === 'reports' && <section><Toolbar title="结论与报告" />{selectedMeeting ? <LiveReportPanel meeting={selectedMeeting} /> : <div style={styles.empty}>暂无可查看报告的评审会。</div>}</section>}
