...
export default function V2AdminPage() {
  ...
  const [projectForm, setProjectForm] = useState({ name: '', submitter: '', description: '', material_statuses: {} as Record<string, string> });
  ...
  const load = async (month = poolMonth) => {
    ...
    const [poolResponse, archiveResponse, purgeResponse, meetingResponse, recycleResponse] = await Promise.all([
      appFetch(`/api/project-pool?scope=active${query}`, { cache: 'no-store' }),
      appFetch(`/api/project-pool?scope=archived${query}`, { cache: 'no-store' }),
      appFetch(`/api/project-pool?scope=purge_pending${query}`, { cache: 'no-store' }),
      appFetch('/api/meetings', { cache: 'no-store' }),
      appFetch('/api/meetings?includeDeleted=true', { cache: 'no-store' })
    ]);
  ...
  const createProject = async () => {
    const response = await appFetch('/api/project-pool', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...projectForm, operator_code: adminCode() }) });
    const data = await response.json(); setNotice(response.ok ? '待评审项目已创建。' : data.error || '创建失败');
    if (response.ok) { setProjectForm({ name: '', submitter: '', description: '', material_statuses: {} }); setShowCreateProject(false); await load(); }
  };
  const createMeeting = async () => {
    const response = await appFetch('/api/meetings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...meetingForm, pool_project_ids: meetingProjectIds, create_projects: quickProjects }) });
  ...
  const setCurrentMeeting = async (meeting: AnyRecord) => {
    const response = await appFetch('/api/meetings', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: meeting.id, is_current: true }) });
  ...
  const recycleMeetings = async (ids: string[]) => {
    const response = await appFetch('/api/meetings/batch', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids, action: 'recycle' }) });
  ...
  const restoreMeetings = async (ids: string[]) => {
    const response = await appFetch('/api/meetings/batch', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids, action: 'restore' }) });
  ...
  const schedule = async (projectIds: string[], meetingId: string) => {
    const response = await appFetch('/api/meeting-assignments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ meeting_id: meetingId, pool_project_ids: projectIds, operator_code: adminCode() }) });
  ...
  const archiveProjects = async (ids: string[]) => {
    ...
    const results = await Promise.all(ids.map((id) => appFetch(`/api/project-pool?id=${encodeURIComponent(id)}&operator_code=${encodeURIComponent(adminCode())}`, { method: 'DELETE' }).then(async (response) => ({ ok: response.ok, data: await response.json() }))));
  ...
  const openProject = async (project: AnyRecord) => {
    ...
    const response = await appFetch(`/api/project-pool/${id}/history`, { cache: 'no-store' }); const data = await response.json();
  ...
  return <main style={styles.page}>
    ...
    {tab === 'reports' && <section><Toolbar title="结论与报告" /><ReportSelector
      ...
      onOpen={(meeting) => window.open(appPath(`/report?meetingId=${encodeURIComponent(meeting.id)}&fromAdmin=true`), '_blank', 'noopener,noreferrer')}
...
function LegacyMeetingWorkspace({ meeting, projects, onBack, onOpenProject, onSchedule, onCurrent, onRefresh, onNotice }: AnyRecord) {
  ...
  const loadWorkspace = async () => { const [projectResponse, summaryResponse] = await Promise.all([appFetch(`/api/projects?meetingId=${meeting.id}&role=admin`, { cache: 'no-store' }), appFetch(`/api/summary?meetingId=${meeting.id}&_=${Date.now()}`, { cache: 'no-store' })]); const projectData = await projectResponse.json(); const summaryData = await summaryResponse.json(); if (projectResponse.ok) setAssignments((projectData.projects || []).sort((a: AnyRecord, b: AnyRecord) => a.seq_no - b.seq_no)); if (summaryResponse.ok) setSummary(summaryData); };
  ...
  const saveMeeting = async () => { const response = await appFetch('/api/meetings', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: meeting.id, ...form }) }); const data = await response.json(); onNotice(response.ok ? '评审会基本信息已保存。' : data.error || '保存失败'); if (response.ok) onRefresh(); };
  const addSelected = async () => { await onSchedule(selected, meeting.id); setSelected([]); await loadWorkspace(); };
  const removeAssignment = async (assignment: AnyRecord) => { if (!window.confirm(`将“${assignment.name}”移出本次评审会？`)) return; const response = await appFetch(`/api/meeting-assignments?id=${assignment.id}&operator_code=${encodeURIComponent(adminCode())}`, { method: 'DELETE' }); const data = await response.json(); onNotice(response.ok ? '项目已移出评审会。' : data.error || '移除失败'); if (response.ok) { await loadWorkspace(); onRefresh(); } };
  const move = async (index: number, offset: number) => { const next = [...assignments]; const target = index + offset; if (target < 0 || target >= next.length) return; [next[index], next[target]] = [next[target], next[index]]; setAssignments(next); const response = await appFetch('/api/meeting-assignments', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ meeting_id: meeting.id, ordered_assignment_ids: next.map((assignment) => assignment.id), operator_code: adminCode() }) }); const data = await response.json(); onNotice(response.ok ? '项目评审顺序已保存。' : data.error || '保存排序失败'); if (!response.ok) await loadWorkspace(); };
...
function LegacyProjectDrawer({ project, onClose, onSaved, onArchive, onNotice }: AnyRecord) {
  ...
  const saveProject = async () => { const response = await appFetch('/api/project-pool', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: project.id, ...form, operator_code: adminCode() }) }); const data = await response.json(); onNotice(response.ok ? '项目详情已保存，列表已同步更新。' : data.error || '保存失败'); if (response.ok) onSaved(project.id); };
  const saveMaterial = async (item: AnyRecord, status: string) => { const response = await appFetch(`/api/project-pool/${project.id}/materials`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ item_key: item.item_key, status, operator_code: adminCode() }) }); const data = await response.json(); onNotice(response.ok ? '资料检查已保存，列表已同步更新。' : data.error || '保存失败'); if (response.ok) onSaved(project.id); };
  const changeStatus = async () => { const response = await appFetch(`/api/project-pool/${project.id}/status`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: manualStatus, note: manualNote, confirmed: true, operator_code: adminCode() }) }); const data = await response.json(); onNotice(response.ok ? '项目状态已人工调整。' : data.error || '调整失败'); if (response.ok) onSaved(project.id); };
