...
export default function LiveReportPanel({ meeting }: { meeting: Item }) {
  ...
  const loadSummary = async (silent = false) => {
      ...
      try {
        ...
        setError(reason.message || '无法读取实时报告数据');
      } finally {
  ...
  useEffect(() => {
    loadSummary();
    const interval = window.setInterval(() => loadSummary(true), 10000);
    return () => {
      window.clearInterval(interval);
    };
  }, [meeting.id]);

  const saveVerdictOverride = async (project: Item, value: string) => {
    const roundId = `r${Number(project.round_no || 1)}`;
    const label = value ? verdictLabels[value] : '跟随推荐';
    if (!window.confirm(`确认将“${project.name}”的结论改为“${label}”？`)) return;
    try {
      const response = await fetch('/api/scores', {
        ...
        body: JSON.stringify({
          ...
          reviewer_code: adminCode(),
          dim_name: `${roundId}::__admin_verdict__`,
          score: 0,
          comment: value || null
        })
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || '保存结论失败');
      await loadSummary(true);
    } catch (reason: any) {
      setError(reason.message || '保存结论失败');
    }
  };
    ...
    {loading ? <div style={styles.empty}>正在读取实时评审数据...</div> : <>
      ...
      <div style={styles.tableWrap}><table style={styles.table}><thead><tr>{['排名', '项目', '本轮总分', '各维度平均分', '完成度', '推荐/当前结论', '盲评参考', '特别推荐票', '问题与行动'].map((label) => <th key={label} style={styles.cell}>{label}</th>)}</tr></thead><tbody>
        {projects.map((project: Item, index: number) => {
          ...
          const verdict = project.verdict;
          const specialVotes = round?.specialVotes || project.specialVotes || [];
          return <tr key={project.id}><td style={styles.cell}>{index + 1}</td><td style={styles.cell}><strong>{project.name}</strong><div style={styles.subtle}>{project.submitter} · 第 {project.round_no || roundNo} 轮</div></td><td style={styles.cell}>{Number(project.totalScore || 0).toFixed(1)} / 100</td><td style={styles.cell}>{renderDimensionAverages(project.dimensionAverages || round?.dimensionAverages)}</td><td style={styles.cell}>{project.completionRate || 0}%</td><td style={styles.cell}><div>{verdictLabels[verdict] || '待形成推荐'}</div><div style={styles.subtle}>推荐：{verdictLabels[project.recommendedVerdict] || '待定'}</div><select aria-label={`${project.name}管理员结论`} value={project.adminVerdict || ''} onChange={(event) => saveVerdictOverride(project, event.target.value)} style={styles.select}><option value="">跟随推荐</option>{Object.entries(verdictLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></td><td style={styles.cell}>{renderBlindStats(round)}</td><td style={styles.cell}>{specialVotes.length ? specialVotes.map((vote: Item) => vote.reviewer_name || vote.reviewer_code).join('、') : '无'}</td><td style={styles.cell}><div>{problems}</div>{actions && <div style={styles.subtle}>行动：{actions}</div>}</td></tr>;
        })}
        {!projects.length && <tr><td colSpan={9} style={styles.empty}>本场评审会尚未形成可汇总的评审数据。</td></tr>}
      </tbody></table></div>
...
function renderDimensionAverages(items: Item[] = []) {
  if (!items.length) return <span style={styles.subtle}>暂无</span>;
  return <div style={styles.dimensionList}>{items.map((item) => <span key={item.name}><strong>{item.name}</strong> {Number(item.averageScore ?? item.score ?? 0).toFixed(1)}/{Number(item.maxScore ?? item.max ?? 0).toFixed(0)} <small>({Number(item.percentage ?? 0).toFixed(0)}%)</small></span>)}</div>;
}

function renderBlindStats(round: Item) {
...
const styles: Record<string, React.CSSProperties> = {
  section: { marginTop: 20 }, heading: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', marginBottom: 14 }, title: { margin: 0, fontSize: 18 }, help: { margin: '5px 0 0', color: '#64748b', fontSize: 13 }, round: { padding: '5px 8px', background: '#f0fdfa', color: '#0f766e', borderRadius: 4, fontSize: 13, fontWeight: 700 }, error: { color: '#b42318', margin: '0 0 12px' }, stats: { display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 10, marginBottom: 14 }, stat: { display: 'grid', gap: 5, padding: 12, border: '1px solid #d9e1ec', borderRadius: 6, background: '#fbfdff' }, tableWrap: { overflowX: 'auto', border: '1px solid #d9e1ec', borderRadius: 6 }, table: { width: '100%', borderCollapse: 'collapse', fontSize: 14 }, cell: { padding: '11px 12px', textAlign: 'left', verticalAlign: 'top', borderBottom: '1px solid #e7edf5' }, subtle: { color: '#64748b', fontSize: 13, marginTop: 4 }, dimensionList: { display: 'grid', gap: 4, minWidth: 180, fontSize: 12 }, empty: { padding: 20, textAlign: 'center', color: '#64748b' }, snapshot: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap', padding: 16, marginTop: 14, border: '1px solid #d9e1ec', borderRadius: 6, background: '#fbfdff' }, primary: { background: '#0f766e', color: '#fff', border: '1px solid #0f766e', padding: '8px 12px', borderRadius: 5, cursor: 'pointer' }, select: { padding: '6px 8px', border: '1px solid #cbd5e1', borderRadius: 5, background: '#fff', marginTop: 6 }, version: { margin: '6px 0 0', color: '#0f766e', fontSize: 13, fontWeight: 700 }
};
