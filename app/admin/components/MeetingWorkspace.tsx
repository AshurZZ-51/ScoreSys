...
export default function MeetingWorkspace({ meeting, projects, source = 'meetings', onBack, onRefresh, onMeetingSaved, onNotice }: {
  ...
  const saveSettings = async () => {
    ...
    if (response.ok) {
      ...
      setForm({ name: saved.name || '', meeting_date: saved.meeting_date || '', deadline: saved.deadline ? String(saved.deadline).slice(0, 10) : '', notes: saved.notes || '' });
  ...
  return <section style={styles.section}><div style={styles.header}><div><button style={styles.secondary} onClick={() => onBack(source)}>返回{source === 'reports' ? '结论与报告' : '评审会列表'}</button><h2 style={{ margin: '12px 0 0', fontSize: 20 }}>{meeting.name}</h2></div><button style={styles.secondary} onClick={() => fetch('/api/meetings', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: meeting.id, is_current: true }) }).then(async (response) => { const data = await response.json(); onNotice(response.ok ? '已设为当前评审会。' : data.error || '设置失败'); if (response.ok) await onRefresh(); })}>设为当前</button></div>
    ...
    {tab === 'summary' && <div style={styles.tableWrap}><table style={styles.table}><thead><tr>{['排名', '项目', '总分', '各维度平均分', '完成度', '推荐/当前结论', '盲评参考', '问题与建议'].map((item) => <th key={item} style={styles.cell}>{item}</th>)}</tr></thead><tbody>{summaryProjects.map((project: Item, index: number) => { const round = project.roundSummaries?.[project.currentRound] || {}; return <tr key={project.id}><td style={styles.cell}>{index + 1}</td><td style={styles.cell}>{project.name}</td><td style={styles.cell}>{Number(project.totalScore || 0).toFixed(1)}</td><td style={styles.cell}>{renderDimensionAverages(project.dimensionAverages || round.dimensionAverages)}</td><td style={styles.cell}>{project.completionRate || 0}%</td><td style={styles.cell}><div>{({ approved: '通过', recheck: '重评', rejected: '不通过' } as Item)[project.verdict] || '待形成推荐'}</div><small style={styles.muted}>推荐：{({ approved: '通过', recheck: '重评', rejected: '不通过' } as Item)[project.recommendedVerdict] || '待定'}</small></td><td style={styles.cell}>{renderBlindStats(round)}{(round.specialVotes || []).length ? <div style={styles.special}>特别推荐：{round.specialVotes.map((vote: Item) => vote.reviewer_name || vote.reviewer_code).join('、')}</div> : null}</td><td style={styles.cell}>{round.problemSummary || '-'}{round.actionSummary ? `；${round.actionSummary}` : ''}</td></tr>; })}{!summaryProjects.length && <tr><td colSpan={8} style={{ ...styles.cell, textAlign: 'center', color: '#64748b' }}>本场评审会尚未形成汇总数据。</td></tr>}</tbody></table></div>}
  </section>;
}

function renderDimensionAverages(items: Item[] = []) {
  if (!items.length) return <span style={styles.muted}>暂无</span>;
