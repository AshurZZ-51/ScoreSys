...
export default function InitiationProjectReport({ report }: { report: Report }) {
  ...
  return <article style={styles.page}>
    <header style={styles.header}>
      <div><p style={styles.kicker}>PROJECT REVIEW DOSSIER</p><h1 style={styles.h1}>立项项目评审报告</h1><p style={styles.subtle}>{project.project_code || project.id || '-'} · {project.name || '-'} · 提报人：{project.submitter || '-'}</p></div>
    ...
    {project.initiation_announcement && <section><h2 style={styles.h2}>立项公示文本</h2><pre style={styles.announcement}>{project.initiation_announcement}</pre></section>}
  </article>;
}

const styles: Record<string, React.CSSProperties> = {
  page: { maxWidth: 1100, margin: '0 auto', padding: 32, background: '#fff', color: '#172033', fontFamily: 'Arial, "Microsoft YaHei", sans-serif' }, header: { display: 'flex', justifyContent: 'space-between', gap: 24, alignItems: 'flex-start', borderBottom: '3px solid #0f766e', paddingBottom: 20, marginBottom: 18 }, kicker: { color: '#0f766e', fontSize: 12, fontWeight: 700, letterSpacing: 1, margin: '0 0 8px' }, h1: { margin: 0, fontSize: 30 }, h2: { fontSize: 18, margin: '28px 0 12px' }, h3: { fontSize: 14, margin: '0 0 12px' }, subtle: { color: '#52606d', margin: '8px 0 0' }, muted: { color: '#64748b', fontSize: 13 }, statusBox: { minWidth: 150, display: 'grid', gap: 5, padding: 14, border: '1px solid #b7d9d5', background: '#f0fdfa' }, overview: { display: 'grid', gridTemplateColumns: '2fr repeat(4, 1fr)', gap: 12 }, tableWrap: { border: '1px solid #d9e1ec', overflowX: 'auto' }, table: { width: '100%', borderCollapse: 'collapse', fontSize: 13 }, empty: { padding: 18, textAlign: 'center', color: '#64748b' }, dimensionGrid: { display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }, dimensionCard: { border: '1px solid #d9e1ec', padding: 16 }, dimension: { marginTop: 12 }, dimensionLine: { display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 13 }, track: { height: 7, background: '#e8eef3', marginTop: 6 }, bar: { display: 'block', height: '100%', background: '#0f766e' }, issueGrid: { display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }, issueCard: { borderLeft: '4px solid #0f766e', padding: '4px 14px', background: '#f8fafc' }, meetingDetail: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, padding: 14, borderTop: '1px solid #d9e1ec' }, reviewerText: { margin: 0, fontSize: 13, lineHeight: 1.7 }, timeline: { display: 'grid', gap: 9, paddingLeft: 20 }, announcement: { whiteSpace: 'pre-wrap', margin: 0, padding: 16, background: '#f8fafc', border: '1px solid #d9e1ec', fontFamily: 'inherit', lineHeight: 1.7 },
};
