...
export default function RoundTwoReport({ report }: { report: Record<string, any> }) {
  const versionGroups = ['two_round_v5', 'two_round_v4', 'two_round_v3', 'two_round_v2'].map((scoringVersion) => ({
    scoringVersion,
    projects: (report?.projects || []).filter((project: Record<string, any>) => (
      (['two_round_v2', 'two_round_v3', 'two_round_v4', 'two_round_v5'].includes(project.scoring_version) ? project.scoring_version : 'two_round_v2') === scoringVersion
    ))
  ...
  return <>
    {versionGroups.map((versionGroup) => {
      ...
      const isV5 = versionGroup.scoringVersion === 'two_round_v5';
      ...
      return <ReportTable
        ...
        title={isV5 ? '第二轮评审报告（当前五维规则）' : isV4 ? '第二轮评审报告（V4 六维历史规则）' : isV3 ? '第二轮评审报告（V3 五维规则）' : '第二轮评审报告（历史规则）'}
        subtitle={isV5 || isV3 ? '游戏性、创新性、项目规划、技术与美术、风险预估' : isV4 ? '游戏性、创新性、项目规划、技术与美术、风险预估、造价与预算' : '项目规划、技术与美术、风险预估'}
