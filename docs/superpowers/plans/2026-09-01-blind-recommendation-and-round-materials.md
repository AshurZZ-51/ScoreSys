...
**Goal:** 在当前 Vercel/Supabase 验收版本中实现全员盲评、推荐结论、管理员覆盖、特别推荐票和两轮资料检查，同时保持现有评分完成度与 PostgreSQL 主线隔离。
...
**Tech Stack:** Next.js 14 App Router, React 18, TypeScript, Supabase JS, PostgreSQL/Supabase SQL migrations, Node.js built-in test runner.

## Global Constraints

- 基线只使用 `codex/project-pool-v2` 的 Supabase 版本，不合并 PostgreSQL 主线。
...
Use the existing Vercel project linkage from `.vercel/project.json` or the repository's configured deployment command. Deploy only after the local build passes, and record the preview URL. Do not deploy or merge the PostgreSQL main branch.
...
Push the verified feature branch to GitHub and company GitLab. Do not merge into company `main` until the PostgreSQL direct-access migration is ready and the company AI deployment target is confirmed for this Supabase-compatible build.
