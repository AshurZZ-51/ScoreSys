...
test('every non-admin reviewer can submit a personal verdict but only Walker advances workflow', () => {
  assert.equal(canSubmitPersonalVerdict({ isAdmin: false, reviewerCode: 'N' }), true);
  assert.equal(canSubmitPersonalVerdict({ isAdmin: true, reviewerCode: 'admin51' }), false);
  assert.equal(shouldAdvanceProjectWorkflow('W'), true);
