...
const MEETING_ASSIGNMENT_PROJECT_FIELDS = 'id, name, submitter, description, status, archived_at';
...
const MATERIAL_ITEMS_BY_ROUND = {
  1: [
    ...
    { item_key: 'mvp_plan', label: 'MVP制作计划', required: true },
    { item_key: 'competitors', label: '竞品或参考', required: false }
  ],
  2: [
    { item_key: 'basic_info', label: '项目基础信息', required: true },
    { item_key: 'risk_statement', label: '风险说明', required: true },
    { item_key: 'mvp_version', label: '可体验 MVP 版本', required: true },
    { item_key: 'initial_plan', label: '项目预算与排期计划', required: true },
    { item_key: 'mvp_description', label: 'MVP版本说明', required: true },
    { item_key: 'virtual_team', label: '虚拟团队资源配置（初步）', required: false },
...
// Legacy callers use this as the initial, creative-stage checklist.
const MATERIAL_ITEMS = MATERIAL_ITEMS_BY_ROUND[1];

function getMaterialItemsForRound(roundNo = 1) {
  return MATERIAL_ITEMS_BY_ROUND[Number(roundNo) === 2 ? 2 : 1];
}
...
function getRoundMaterials(materials, roundNo) {
  const rows = materials || [];
  if (rows.some((item) => item.round_no != null)) {
    return rows.filter((item) => Number(item.round_no || 1) === Number(roundNo));
  }
  const roundKeys = new Set(getMaterialItemsForRound(roundNo).map((item) => item.item_key));
  return rows.filter((item) => roundKeys.has(item.item_key));
}

function getMaterialStatus(materials, roundNo = 1) {
  const statusByKey = new Map(getRoundMaterials(materials, roundNo).map((item) => [item.item_key, item.status]));
  const missing = getMaterialItemsForRound(roundNo).filter((item) => item.required && !['submitted', 'exempt'].includes(statusByKey.get(item.item_key)));
...
function getMaterialProgress(materials, roundNo = 1) {
  const required = getMaterialItemsForRound(roundNo).filter((item) => item.required);
  const statusByKey = new Map(getRoundMaterials(materials, roundNo).map((item) => [item.item_key, item.status]));
...
function createMaterialRows(projectId, initialStatuses = {}, checkedBy = null, checkedAt = null, roundNo = 1) {
  return getMaterialItemsForRound(roundNo).map((item) => ({
    project_id: projectId,
    item_key: item.item_key,
    round_no: Number(roundNo) === 2 ? 2 : 1,
    required: item.required,
    status: isMaterialStatus(initialStatuses[item.item_key]) ? initialStatuses[item.item_key] : 'missing',
    ...(isMaterialStatus(initialStatuses[item.item_key]) && initialStatuses[item.item_key] !== 'missing' && checkedBy ? { checked_by: checkedBy, checked_at: checkedAt } : {})
  }));
}

function buildMaterialUpsert(projectId, itemKey, required, status, note, checkedBy, checkedAt, roundNo = 1) {
  return { project_id: projectId, item_key: itemKey, round_no: Number(roundNo) === 2 ? 2 : 1, required, status, note, checked_by: checkedBy, checked_at: checkedAt };
}
...
function assignmentRoundForStatus(status) {
  return String(status || '').includes('r2') ? 2 : 1;
}

function getReviewableMeetingProjects(projects) {
  return (projects || []).filter((project) => project?.name && project?.submitter);
}
...
module.exports = {
  ...
  MEETING_ASSIGNMENT_PROJECT_FIELDS,
  MATERIAL_STATUSES,
  MATERIAL_ITEMS,
  MATERIAL_ITEMS_BY_ROUND,
  PROJECT_STATUS_LABELS,
  normalizeProjectPart,
  makeMatchKey,
  getMaterialItemsForRound,
  getMaterialStatus,
  isMaterialStatus,
  getMaterialProgress,
  projectStatusLabel,
