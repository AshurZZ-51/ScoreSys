...
test('project pool constants expose required materials and capacity', () => {
  ...
  assert.equal(workflow.MATERIAL_ITEMS.length, 10);
  assert.equal(workflow.MATERIAL_ITEMS.filter((item) => item.required).length, 7);
  assert.equal(workflow.MATERIAL_ITEMS.find((item) => item.item_key === 'operations_metrics').required, true);
  assert.equal(workflow.MATERIAL_ITEMS.find((item) => item.item_key === 'virtual_team').required, true);
});
...
test('material progress exposes the required approval count for list displays', () => {
  ...
  assert.deepEqual(workflow.getMaterialProgress(materials), { approved: 1, total: 7, complete: false });
});
...
test('builds material updates around the composite project and item key', () => {
  assert.deepEqual(workflow.buildMaterialUpsert('project-1', 'basic_info', true, 'submitted', '', 'admin51', '2026-08-12T00:00:00.000Z'), {
