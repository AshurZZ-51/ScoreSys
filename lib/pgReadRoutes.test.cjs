...
const hostedAdminName = ['sup', 'abase', 'Admin'].join('');

const routePaths = [
  'app/api/auth/login/route.ts',
  'app/api/accounts/route.ts',
  'app/api/accounts/[code]/route.ts',
  'app/api/meetings/batch/route.ts',
...
test('migrated read paths call repositories without hosted client access', () => {
  const methods = [
  ...
  for (const [route, method, nextMethod] of methods) {
    ...
    assert.equal(body.includes(hostedAdminName), false, `${route} ${method}`);
  }
...
test('simple write handlers use repositories without hosted client access', () => {
  for (const [route, methods] of [
    ['app/api/accounts/route.ts', ['GET', 'POST']],
    ['app/api/accounts/[code]/route.ts', ['PATCH']],
    ['app/api/meetings/batch/route.ts', ['POST']],
    ['app/api/meetings/delete/route.ts', ['POST']],
  ...
  assert.match(source('app/api/scores/route.ts'), /@\/lib\/db\/repositories\/scoreWorkflow/);
});
