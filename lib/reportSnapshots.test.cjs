...
test('report type query values preserve the selected round and default safely', () => {
  assert.equal(reports.resolveReportType('round_2'), 'round_2');
  assert.equal(reports.resolveReportType('round_1'), 'round_1');
  assert.equal(reports.resolveReportType('unexpected'), 'round_1');
});

test('meeting report rankings contain projects with resolved recommendations', () => {
  ...
  assert.deepEqual(payload.projects.map((project) => project.id), ['admin-only', 'high', 'low']);
  assert.equal(payload.projects[0].rank, 1);
  assert.equal(payload.projects[1].verdict, 'recheck');
});
