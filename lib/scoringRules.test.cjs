...
test('special recommendation votes accept only zero or one', () => {
...
test('computes version-three round two from all five weighted dimensions', () => {
  const scores = [];
  for (const rule of rules.getRoundScoringDimensions('r2', 'two_round_v3')) {
    if (rule.type === 'level') {
      scores.push({ reviewer_code: 'W', dim_name: rules.roundScoreKey('r2', rule.name, 'level'), score: 20 });
      continue;
    }
    for (const item of rule.items) {
      scores.push({ reviewer_code: 'W', dim_name: rules.roundScoreKey('r2', rule.name, item.key), score: 10 });
    }
  }

  const result = rules.computeRoundProjectScore('r2', scores, 0, 'two_round_v3');

  assert.deepEqual(rules.getRoundDefinition('r2', 'two_round_v3').dimensions, ['游戏性', '创新性', '项目规划', '技术&美术', '风险预估']);
  assert.equal(result.baseScore, 100);
  assert.equal(result.dimTotals['游戏性'].maxScore, 30);
  assert.equal(result.dimTotals['创新性'].maxScore, 20);
  assert.equal(result.dimTotals['项目规划'].maxScore, 20);
  assert.equal(result.dimTotals['技术&美术'].maxScore, 15);
  assert.equal(result.dimTotals['风险预估'].maxScore, 15);
  assert.equal(rules.expectedInputCountForRound('r2', 'two_round_v3'), 16);
});

test('keeps legacy second-round innovation levels and weights for version-two records', () => {
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 30, 'two_round_v2'), false);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 8, 'two_round_v3'), true);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 30, 'two_round_v3'), false);
  assert.equal(rules.getRoundScoringDimensions('r2', 'two_round_v2').length, 3);
});

test('keeps Walker bonus separate from the 100-point round base score', () => {
  const scores = [];
  for (const rule of rules.getRoundScoringDimensions('r2', 'two_round_v3')) {
    if (rule.type === 'level') {
      scores.push({ reviewer_code: 'W', dim_name: rules.roundScoreKey('r2', rule.name, 'level'), score: 20 });
    } else {
      rule.items.forEach((item) => scores.push({ reviewer_code: 'W', dim_name: rules.roundScoreKey('r2', rule.name, item.key), score: 10 }));
    }
  }

  const result = rules.computeRoundProjectScore('r2', scores, 5, 'two_round_v3');

  assert.equal(result.baseScore, 100);
  assert.equal(result.bonusScore, 5);
  assert.equal(result.totalScore, 105);
});

test('computes version-four round two from six weighted dimensions', () => {
  const dimensions = rules.getRoundScoringDimensions('r2', 'two_round_v4');
  assert.deepEqual(dimensions.map((rule) => [rule.name, rule.maxScore]), [
    ['游戏性', 25],
    ['创新性', 20],
    ['项目规划', 15],
    ['技术&美术', 15],
  ...
  const result = rules.computeRoundProjectScore('r2', scores, 0, 'two_round_v4');
  assert.equal(result.baseScore, 100);
  assert.equal(result.dimTotals['造价与预算'].maxScore, 10);
  assert.equal(rules.expectedInputCountForRound('r2', 'two_round_v4'), 18);
  assert.equal(rules.getRoundDefinition('r2', 'two_round_v4').title, '游戏性/创新性/项目规划/技术&美术/风险预估/造价与预算');
});

test('validates version-four innovation levels without changing version-three', () => {
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 8, 'two_round_v4'), true);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 20, 'two_round_v4'), true);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 16, 'two_round_v4'), false);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 8, 'two_round_v3'), true);
  assert.equal(rules.getRoundScoringDimensions('r2', 'two_round_v3').length, 5);
});

test('current version-five second round uses the confirmed five-dimension weights', () => {
  const dimensions = rules.getRoundScoringDimensions('r2', 'two_round_v5');
  assert.deepEqual(dimensions.map((rule) => [rule.name, rule.maxScore, rule.multiplier]), [
    ['游戏性', 30, 3],
    ['创新性', 20, undefined],
    ['项目规划', 20, 2],
    ['技术&美术', 15, 1.5],
  ...
  assert.deepEqual(rules.getRoundDefinition('r2', 'two_round_v5').dimensions, [
    '游戏性', '创新性', '项目规划', '技术&美术', '风险预估'
  ]);
  assert.equal(rules.expectedInputCountForRound('r2', 'two_round_v5'), 16);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 20, 'two_round_v5'), true);
  assert.equal(rules.isValidScoreValue('r2::创新性::level', 16, 'two_round_v5'), false);
});
