const COMPLETED_VERDICTS = new Set(['approved', 'recheck', 'rejected']);
const { expectedInputCountForRound, isNormalScoringKey } = require('./scoringRules');

function resolveReportScoringVersion(value) {
...
function isCompletedWalkerReview(project) {
  return COMPLETED_VERDICTS.has(
    project?.verdict
      ?? project?.adminVerdict
      ?? project?.recommendedVerdict
      ?? project?.walkerVerdict
      ?? project?.walker_verdict
  );
}
...
function resolveReportType(value) {
  return value === 'round_2' ? 'round_2' : 'round_1';
}
...
function reportProject(project, rank) {
  return {
    ...
    scoring_version: resolveReportScoringVersion(project.scoring_version),
    rank,
    ...
    dimensionAverages: project.dimensionAverages || project.roundSummaries?.[`r${project.round_no}`]?.dimensionAverages || [],
    blindRatingStats: project.roundSummaries?.[`r${project.round_no}`]?.blindRatingStats || {},
    blindVerdictStats: project.roundSummaries?.[`r${project.round_no}`]?.blindVerdictStats || {},
    ...
    verdict: project.verdict
      ...
      ?? project.walkerVerdict
      ?? project.walker_verdict,
    recommendedVerdict: project.recommendedVerdict || null,
    adminVerdict: project.adminVerdict || null,
    specialVotes: project.specialVotes || project.roundSummaries?.[`r${project.round_no}`]?.specialVotes || []
  };
}
...
function buildMeetingReviewerStats(summary, projects) {
  ...
  const versionByProject = new Map(selectedProjects.map((project) => [
    project.id,
    resolveReportScoringVersion(project.scoring_version)
  ]));
...
function buildMeetingReportPayload(summary, meeting, reportType) {
  ...
  const completedProjects = (summary?.projects || [])
    .filter((project) => !roundNo || Number(project.round_no || 1) === roundNo)
    .filter(isCompletedWalkerReview);
  const rankByProjectId = new Map();
  const versions = new Set(completedProjects.map((project) => resolveReportScoringVersion(project.scoring_version)));
  versions.forEach((version) => {
    completedProjects
      .filter((project) => resolveReportScoringVersion(project.scoring_version) === version)
      .sort((left, right) => Number(right.totalScore || 0) - Number(left.totalScore || 0))
      .forEach((project, index) => rankByProjectId.set(project.id, index + 1));
  });
  const projects = completedProjects
    .sort((left, right) => Number(right.totalScore || 0) - Number(left.totalScore || 0))
    .map((project) => reportProject(project, rankByProjectId.get(project.id) || 0));
  ...
  return {
    ...
    meeting: {
      ...
      deadline: meeting?.deadline || summary?.meeting?.deadline || '',
...
function buildInitiationProjectPayload(project, summaries, timeline) {
  ...
  return {
    ...
    project: {
      ...
      preliminary_rating: project?.preliminary_rating || '',
      final_rating: project?.final_rating || '',
      initiation_announcement: project?.initiation_announcement || ''
    },
    roundHistory,
    timeline: timeline || []
  };
