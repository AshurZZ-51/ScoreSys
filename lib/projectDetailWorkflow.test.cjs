...
test('keeps the special recommendation vote independent from the personal verdict', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'app', 'scoring', 'page.tsx'), 'utf8');
  assert.match(source, /specialScoreKey\(roundId, '__verdict__'\)/);
  assert.match(source, /specialScoreKey\(roundId, '__special_vote__'\)/);
  const specialVoteLine = source.split('\n').find((line) => line.includes('aria-label="特别推荐票"'));
  assert.ok(specialVoteLine, 'special recommendation vote control should exist');
  assert.doesNotMatch(specialVoteLine, /disabled=\{saving\}/);
