...
function getReviewVerdict(assignment) {
  const directVerdict = assignment?.verdict
    ?? assignment?.recommended_verdict
    ?? assignment?.recommendedVerdict
    ?? assignment?.admin_verdict
    ?? assignment?.adminVerdict
    ?? assignment?.walker_verdict
    ?? assignment?.walkerVerdict;
  if (directVerdict !== undefined && directVerdict !== null) return directVerdict;
  const verdictScore = (assignment?.scores || []).find((score) => (
    !String(score?.reviewer_code || '').toLowerCase().startsWith('admin')
    && String(score?.dim_name || '').includes('__verdict__')
    && COMPLETED_VERDICTS.has(score?.comment)
  ));
  return verdictScore?.comment;
}

function isCompletedReview(assignment) {
  return COMPLETED_VERDICTS.has(getReviewVerdict(assignment));
}
