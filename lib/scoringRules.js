...
function parseScoreKey(dimName) {
  ...
  const rule = DIMENSION_BY_NAME[dimensionName];
  if (!rule) return null;

  if (!itemKey) {
    return { dimensionName, itemKey: null, rule, legacy: true };
  }

  if (rule.type === 'level') {
    return itemKey === 'level' ? { dimensionName, itemKey, rule, legacy: false } : null;
  }

  const exists = rule.items.some((item) => item.key === itemKey);
  return exists ? { dimensionName, itemKey, rule, legacy: false } : null;
}
...
function computeDimensionResult(dimensionName, scores) {
  ...
  if (rule.type === 'level') {
    const values = scores
      .filter((score) => parseScoreKey(score.dim_name)?.dimensionName === dimensionName)
  ...
  const itemResults = rule.items.map((item) => {
    const values = scores
      .filter((score) => score.dim_name === scoreKey(dimensionName, item.key))
      .map((score) => Number(score.score))
