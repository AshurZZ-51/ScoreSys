...
const ROUND_TWO_V4_DIMENSIONS = [
  ...
  },
  {
    ...
    multiplier: 1,
    items: [
      { key: 'budget_reasonableness', label: '预算合理性' },
      { key: 'cost_control', label: '成本控制' },
      { key: 'roi', label: '投入产出预期' }
    ]
  }
];

// Current second-round rules. V4 remains readable for historical snapshots.
const ROUND_TWO_V5_DIMENSIONS = ROUND_TWO_V3_DIMENSIONS.map((rule) => ({
  ...rule,
...
const ROUND_RULESETS = {
  ...
  two_round_v4: {
    r1: SCORING_DIMENSIONS.filter((rule) => rule.roundId === 'r1'),
    r2: ROUND_TWO_V4_DIMENSIONS
  },
  two_round_v5: {
    r1: SCORING_DIMENSIONS.filter((rule) => rule.roundId === 'r1'),
    r2: ROUND_TWO_V5_DIMENSIONS
  }
};
...
const ROUND_DEFINITIONS = {
  ...
  two_round_v4: [
    { id: 'r1', label: '第一轮', title: '游戏性/创新性', dimensions: ['游戏性', '创新性'] },
    { id: 'r2', label: '第二轮', title: '游戏性/创新性/项目规划/技术&美术/风险预估/造价与预算', dimensions: ['游戏性', '创新性', '项目规划', '技术&美术', '风险预估', '造价与预算'] }
  ],
  two_round_v5: [
    { id: 'r1', label: '第一轮', title: '游戏性/创新性', dimensions: ['游戏性', '创新性'] },
    { id: 'r2', label: '第二轮', title: '游戏性/创新性/项目规划/技术&美术/风险预估', dimensions: ['游戏性', '创新性', '项目规划', '技术&美术', '风险预估'] }
...
const REVIEW_ROUNDS = ROUND_DEFINITIONS.two_round_v5;
/* Keep historical assignments on their recorded ruleset; new round-two assignments use v5. */
...
function resolveScoringVersion(scoringVersion) {
  if (scoringVersion === 'two_round_v5') return 'two_round_v5';
  if (scoringVersion === 'two_round_v4') return 'two_round_v4';
  if (scoringVersion === 'two_round_v3') return 'two_round_v3';
  return 'two_round_v2';
}
...
function getScoreMax(dimName, scoringVersion = DEFAULT_SCORING_VERSION) {
  ...
  if (baseDimName === '__special_vote__') return 1;
...
module.exports = {
  SCORING_DIMENSIONS,
  ROUND_TWO_V4_DIMENSIONS,
  ROUND_TWO_V5_DIMENSIONS,
