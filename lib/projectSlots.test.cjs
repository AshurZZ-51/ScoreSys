...
test('creates only missing template slots for an existing meeting', () => {
  const existingProjects = [1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12].map((seq_no) => ({ seq_no }));

  const missing = slots.getMissingTemplateProjects(existingProjects, 'meeting-3');

  assert.equal(missing.length, 1);
  assert.equal(missing[0].seq_no, 9);
  assert.equal(missing[0].meeting_id, 'meeting-3');
  assert.equal(missing[0].is_template, true);
});
