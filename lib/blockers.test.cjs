...
test('final runtime image installs a pinned PostgreSQL client and exposes psql', () => {
  const dockerfile = read('Dockerfile');
  assert.match(dockerfile, /apk add --no-cache postgresql(?:16)?-client(?:=|\s)/i);
  assert.match(dockerfile, /command -v psql|psql --version/);
});

test('CCE import and CI stream one validated bundle after migration', () => {
  const template = read('ops/cce/job-import.yaml.tmpl');
  const ci = read('.gitlab-ci.yml');
  assert.doesNotMatch(template, /--fail-fast/);
  assert.match(template, /import-snapshot\.mjs", "--force", "--file", "-"/);
  assert.match(ci, /SNAPSHOT_FILE/);
  assert.match(ci, /MANIFEST_FILE/);
  assert.match(ci, /jq/);
  assert.match(ci, /--force --file -/);
  const importNeeds = ci.match(/import-db:[\s\S]*?\n\n[^ ]/)[0];
  assert.match(importNeeds, /job: migrate-db/);
});
