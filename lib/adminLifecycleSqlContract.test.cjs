...
test('workflow repair migration keeps database assignment and manual-status rules aligned with the admin UI', () => {
  const migration = fs.readFileSync(path.join(__dirname, '..', 'MIGRATION_WORKFLOW_FIX_V4.sql'), 'utf8');

  assert.match(migration, /CREATE OR REPLACE FUNCTION apply_project_pool_mutations\s*\(/);
  assert.match(migration, /ELSE updated\.status END/);
  assert.match(migration, /'draft', 'materials_pending', 'ready_r1'/);
  assert.match(migration, /CREATE OR REPLACE FUNCTION assign_pool_project_to_meeting\s*\(/);
  assert.match(migration, /pool_row\.status IN \('draft', 'materials_pending', 'ready_r1'\)/);
});
