...
test('reorderMeetingAssignments moves the dragged assignment to its target without mutating the source', () => {
  const assignments = [{ id: 'one' }, { id: 'two' }, { id: 'three' }];

  assert.deepEqual(
    lifecycle.reorderMeetingAssignments(assignments, 'one', 'three').map((assignment) => assignment.id),
    ['two', 'three', 'one']
  );
  assert.deepEqual(assignments.map((assignment) => assignment.id), ['one', 'two', 'three']);
});
...
test('countCompletedReviews counts any submitted reviewer verdict', () => {
  const assignments = [
  ...
  assert.equal(lifecycle.countCompletedReviews(assignments), 3);
});

test('project pool views distinguish workflow state from completed Walker review evidence', () => {
  const scheduled = { status: 'scheduled_r1', projects: [] };
  const completed = { status: 'ready_r2', projects: [{ scores: [{ reviewer_code: 'W', dim_name: 'r1::__verdict__', comment: 'approved' }] }] };
  const manualOnly = { status: 'ready_r2', latest_verdict: 'approved', projects: [] };

  assert.equal(lifecycle.isPendingReviewProject(scheduled), true);
  assert.equal(lifecycle.hasCompletedReview(completed), true);
  assert.equal(lifecycle.isPendingReviewProject(completed), true);
  assert.equal(lifecycle.hasCompletedReview(manualOnly), false);
});
