...
function getMissingTemplateProjects(existingProjects, meetingId) {
  const existingSeqNos = new Set((existingProjects || []).map((project) => Number(project.seq_no)));
  return createTemplateProjects(meetingId).filter((project) => !existingSeqNos.has(project.seq_no));
}
